...
// Module entry point
extern "C" int udp(tModInfo *modInfo) {
  NBBOTS = 10;
  Drivers.clear();
  pathBuffer = "drivers/udp/dandroid.xml";
  nameBuffer = "udp";
...
// Start a new race.
static void newRace(int index, tCarElt* car, tSituation *s) {
  driver[index]->NewRace(car, s, index);
}


// Drive during race.
static void drive(int index, tCarElt* car, tSituation *s) {
  driver[index]->Drive(index);
}


// Pitstop callback.
static int pitcmd(int index, tCarElt* car, tSituation *s) {
  return 0;//driver[index]->PitCmd();
}
